...
def expense(): # expense function allows user to record bills and expenses that have been incured on their account. 
    ...
    if confirm == "yes":
        ...
        print(f"Balance after expenses: ${new_balance}")
        current_time = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
        with open("transaction_history.txt", "a") as history_file:
            for expense in expenses:
                expense_type, expense_amount = expense
                history_file.write(f"Date: {current_time}, Expense Type: {expense_type}, Expense Amount: {expense_amount}, Balance Before: {round(balance, 2)}, Balance After: {round(balance - expense_amount, 2)}\n")
...
def main():
    while True:
        print("current Balance: {0}".format(round(balance, 2))) # changed to always show balance instead of user deciding to view balance. 
